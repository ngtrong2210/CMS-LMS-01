...
public sealed class SystemTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task VideoUpload_SavesInsideProject_AndServesStaticRelativeUrl()
    {
        ...
        var storage = _factory.Services.GetRequiredService<SimsObject.Interfaces.IVideoStorageService>();
    ...
    {
        var storage = _factory.Services.GetRequiredService<SimsObject.Interfaces.IProjectStorageService>();
        var environment = _factory.Services.GetRequiredService<IWebHostEnvironment>();
        var contentRoot = Path.GetFullPath(environment.ContentRootPath) + Path.DirectorySeparatorChar;
        var created = new List<string>();
        try
        {
            foreach (var area in Enum.GetValues<SimsObject.Interfaces.ProjectStorageArea>())
            {
            ...
            Assert.Equal(1, await storage.CleanupExpiredAsync(SimsObject.Interfaces.ProjectStorageArea.Cache, TimeSpan.FromHours(24)));
            Assert.False(storage.Exists(created[0]));
            created.RemoveAt(0);

            var videoStorage = _factory.Services.GetRequiredService<SimsObject.Interfaces.IVideoStorageService>();
