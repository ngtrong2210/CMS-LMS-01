<template>
  <div class="interactive-player">
    ...
    <div v-if="source" class="interaction-timeline">
      <div class="interaction-track" :class="{seekable:canTimelineSeek}" role="slider" :tabindex="canTimelineSeek?0:-1" aria-label="Dòng thời gian câu hỏi tương tác" aria-valuemin="0" :aria-valuemax="effectiveDuration" :aria-valuenow="Math.round(currentTime)" @click="seekFromTimeline" @keydown.left.prevent="stepTimeline(-5)" @keydown.right.prevent="stepTimeline(5)">
        ...
        <button v-for="(item,index) in normalized" :key="item.id" type="button" class="timeline-question" :class="{answered:item.answered,staggered:index%2===1}" :style="{left:markerPercent(item.timeSeconds)+'%'}" :title="`${formatTime(item.timeSeconds)} — ${item.label}`" @click.stop="openQuestion(item)"><i :class="['bi',item.answered?'bi-check-circle-fill':'bi-patch-question-fill']"></i><span>{{ formatTime(item.timeSeconds) }}</span></button>
      </div>
      <div class="timeline-current"><i class="bi bi-cursor-fill"></i> Đang xem <strong>{{ formatTime(currentTime) }}</strong></div>
    </div>
...
const videoRef=ref(null),activeQuestion=ref(null),singleAnswer=ref(''),multipleAnswers=ref([]),shortAnswer=ref(''),submitting=ref(false),answerError=ref(''),answerResult=ref(null),watchedMax=ref(props.maxWatchedTime),loaded=ref(false),currentTime=ref(props.initialTime)
...
const currentPercent=computed(()=>effectiveDuration.value?Math.min(100,Math.max(0,currentTime.value/effectiveDuration.value*100)):0)
...
function resetPlayer(){const element=videoRef.value;element?.pause();watchedMax.value=props.maxWatchedTime;currentTime.value=0;resetEngine(0);if(element){element.currentTime=0;element.playbackRate=1}}
function onLoaded(){loaded.value=true;const element=videoRef.value;if(!element)return;const duration=Number.isFinite(element.duration)?element.duration:props.durationSeconds;element.currentTime=Math.min(Math.max(0,props.initialTime),Math.max(0,duration-.25));currentTime.value=element.currentTime;engine.loadAt(element.currentTime);emitProgress()}
...
function onTimeUpdate(){const element=videoRef.value;if(!element||!loaded.value)return;currentTime.value=element.currentTime;watchedMax.value=Math.max(watchedMax.value,element.currentTime);activate(engine.tick(element.currentTime));emitProgress()}
...
function onSeeked(){const element=videoRef.value;if(!element)return;currentTime.value=element.currentTime;engine.endSeek(element.currentTime);emitProgress()}
...
function markerPercent(time){return effectiveDuration.value?Math.min(99,Math.max(1,time/effectiveDuration.value*100)):0}
function seekTo(time){if(!videoRef.value)return;const target=Math.min(effectiveDuration.value,Math.max(0,Number(time)||0));currentTime.value=target;videoRef.value.currentTime=target}
function seekFromTimeline(event){if(!canTimelineSeek.value)return;const rect=event.currentTarget.getBoundingClientRect();if(!rect.width)return;seekTo((event.clientX-rect.left)/rect.width*effectiveDuration.value)}
function stepTimeline(seconds){if(canTimelineSeek.value)seekTo(currentTime.value+seconds)}
...
<style scoped>
.interactive-player{height:100%;min-height:0;display:grid;grid-template-rows:minmax(0,1fr) 92px;overflow:hidden;background:#fff}.interactive-stage{height:auto;min-height:0;overflow:hidden;background:#071922;position:relative;display:grid;place-items:center}.interactive-stage video{display:block;width:100%;height:100%;min-height:0;object-fit:contain;background:#000}.interactive-empty{display:grid;place-items:center;gap:.6rem;color:#d5e1dc}.interactive-empty i{font-size:3rem;color:#ffc224}.interactive-empty span{color:#93a8a0;font-size:.82rem}.interaction-timeline{min-width:0;padding:.7rem 1.15rem .4rem;background:#fff;border-top:1px solid #edf1ef}.interaction-track{height:12px;margin:.4rem .25rem 2.25rem;border-radius:6px;background:#e3ebe8;position:relative;outline:none}.interaction-track.seekable{cursor:pointer}.interaction-track:focus-visible{box-shadow:0 0 0 4px rgba(0,80,153,.14)}.interaction-track-progress{position:absolute;inset:0 auto 0 0;border-radius:6px;background:#c9dcd5;pointer-events:none}.interaction-playhead{position:absolute;top:50%;z-index:3;width:4px;height:28px;border-radius:4px;background:var(--eduvers-black);transform:translate(-50%,-50%);box-shadow:0 2px 7px rgba(0,80,153,.28);pointer-events:none}.timeline-question{position:absolute;top:50%;z-index:4;transform:translate(-50%,-50%);border:0;background:transparent;color:var(--eduvers-base);padding:0;font-size:1.18rem;line-height:1}.timeline-question.answered{color:var(--eduvers-base)}.timeline-question>span{position:absolute;top:19px;left:50%;transform:translateX(-50%);color:#52645e;font-size:.65rem;white-space:nowrap}.timeline-question.staggered>span{top:30px}.timeline-current{display:flex;justify-content:flex-end;align-items:center;gap:.4rem;color:#63736d;font-size:.78rem}.timeline-current i{color:var(--eduvers-base)}.timeline-current strong{color:var(--eduvers-black);font-size:.88rem}.interaction-backdrop{position:fixed;inset:0;background:rgba(10,31,24,.66);z-index:2000;display:grid;place-items:center;padding:1rem}.interaction-modal{width:min(590px,100%);padding:2rem;position:relative;max-height:92vh;overflow:auto}.interaction-modal h2{font-size:1.4rem;font-weight:750;margin-top:1rem}.interaction-close{position:absolute;right:1rem;top:1rem;border:0;background:#f3f6f5;width:34px;height:34px;border-radius:8px}.interaction-answer{display:flex;align-items:center;gap:.75rem;padding:.8rem;margin:.55rem 0;background:#f7f9f8;border-radius:9px}.interaction-answer>span{width:28px;height:28px;border-radius:7px;background:white;display:grid;place-items:center;font-weight:750}.interaction-result{text-align:center;padding-top:1rem}.interaction-result>i{font-size:3.5rem}.interaction-result.correct{color:var(--eduvers-base)}.interaction-result.wrong{color:var(--eduvers-base)}.interaction-result.pending{color:#8a7000}.interaction-result h3{font-size:1.3rem;margin:.6rem 0}.interaction-result p{color:#53655f;margin-top:1rem}@media(max-width:600px){.interactive-player{grid-template-rows:minmax(0,1fr) 86px}.interaction-timeline{padding-left:.75rem;padding-right:.75rem}.timeline-current{font-size:.72rem}}
.interaction-result.correct{color:#07875a}.interaction-result.wrong{color:#cd1b1b}
.interaction-track:focus-visible{box-shadow:0 0 0 4px rgba(var(--eduvers-blue-rgb),.14)}.interaction-playhead{background:var(--eduvers-blue);box-shadow:0 2px 7px rgba(var(--eduvers-blue-rgb),.28)}.timeline-question,.timeline-question.answered,.timeline-current i{color:var(--eduvers-blue)}
</style>
