    ...
    <div class="row">
      ...
      <div class="col-xl-8 col-lg-7">
        <div class="course-list__right">
          ...
          <div v-else-if="error" class="alert alert-danger">{{ error }} <button class="btn btn-sm btn-light ms-2" @click="loadCourses">Thử lại</button></div>
          ...
          <article v-for="(course,index) in filtered" :key="course.id" class="course-list__single">
            ...
            <div class="course-list__content">
              <div class="course-list__doller-and-heart">
                <div class="course-list__doller"><p>{{ course.progress }}% hoàn thành</p></div>
                <div class="course-list__heart"><RouterLink :to="`/lms/courses/${course.id}`"><i class="bi bi-bookmark"></i></RouterLink></div>
              </div>
              <h2 class="course-list__title"><RouterLink :to="`/lms/courses/${course.id}`">{{ course.title }}</RouterLink></h2>
              ...
              <ul class="course-list__meta list-unstyled">
                <li><p><span><i class="bi bi-bar-chart"></i></span>{{ course.enrollmentStatus === 'COMPLETED' ? 'Hoàn thành' : 'Đang học' }}</p></li>
                <li><p><span><i class="bi bi-book"></i></span>{{ course.lessons }} bài học</p></li>
                <li><p><span><i class="bi bi-clock"></i></span>Học linh hoạt</p></li>
              </ul>
              <div class="course-list__btn-and-client-info">
                <div class="course-list__btn-box">
                  <RouterLink :to="`/lms/courses/${course.id}`" class="thm-btn">Xem khóa học <i class="bi bi-arrow-right"></i></RouterLink>
                </div>
                <div class="course-list__client-box">
                  <div class="course-list__client-img"><img :src="teacherImages[index % teacherImages.length]" :alt="course.teacher"></div>
                  <div class="course-list__client-content"><h4>{{ course.teacher }}</h4><p>Giảng viên</p></div>
                </div>
              </div>
            </div>
          </article>
...
const courseImages=[courseList1,courseList2,courseList3,courseList4,courseList5],teacherImages=[teacher1,teacher2,teacher3,teacher4,teacher5]
...
async function loadCourses(){loading.value=true;error.value='';try{const data=await axiosClient.get('/lms/courses',{params:{_fresh:Date.now()}});courses.value=(Array.isArray(data)?data:[]).map((row,index)=>({id:Number(pick(row,'Id','id')),code:pick(row,'Code','code')||'',title:pick(row,'Title','title')||'',teacher:pick(row,'TeacherName','teacherName')||'',category:pick(row,'CategoryName','categoryName')||'Khóa học',lessons:Number(pick(row,'LessonCount','lessonCount')||0),progress:Number(pick(row,'ProgressPercent','progressPercent')||0),enrollmentStatus:pick(row,'EnrollmentStatus','enrollmentStatus')||'ENROLLED',color:colors[index%colors.length]}))}catch(e){error.value=e.message}finally{loading.value=false}}
</script>
