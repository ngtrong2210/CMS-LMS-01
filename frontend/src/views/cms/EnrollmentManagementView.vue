<template>
  <section>
    <header class="d-flex flex-wrap justify-content-between align-items-end gap-3 mb-4">
      ...
      <button class="btn btn-brand" @click="openForm">
    ...
    <div class="summary-grid mb-3">
      <article class="app-card summary-card"><span class="summary-icon"><i class="bi bi-people"></i></span><div><small>Đang tham gia</small><strong>{{ activeCount }}</strong></div></article>
      <article class="app-card summary-card"><span class="summary-icon secondary"><i class="bi bi-mortarboard"></i></span><div><small>Khóa học có học viên</small><strong>{{ courseCount }}</strong></div></article>
      <article class="app-card summary-card"><span class="summary-icon soft"><i class="bi bi-check-circle"></i></span><div><small>Đã hoàn thành</small><strong>{{ completedCount }}</strong></div></article>
    </div>

    <div class="app-card p-3 mb-3">
      <div class="row g-2">
        <div class="col-lg-6"><div class="search-box"><i class="bi bi-search"></i><input v-model.trim="search" class="form-control" placeholder="Tìm học viên, mã học viên hoặc khóa học..."></div></div>
        <div class="col-md-3"><select v-model="courseFilter" class="form-select"><option value="">Tất cả khóa học</option><option v-for="course in courses" :key="course.id" :value="course.id">{{ course.code }} — {{ course.title }}</option></select></div>
        <div class="col-md-3"><select v-model="statusFilter" class="form-select"><option value="">Tất cả trạng thái</option><option value="ENROLLED">Đã ghi danh</option><option value="IN_PROGRESS">Đang học</option><option value="COMPLETED">Hoàn thành</option><option value="CANCELLED">Đã hủy</option></select></div>
      </div>
    </div>
    ...
    <div class="app-card p-2">
      <div class="table-responsive">
        <table class="table align-middle mb-0">
          <thead><tr><th>Học viên</th><th>Khóa học</th><th>Ngày ghi danh</th><th>Tiến độ</th><th>Điểm</th><th>Trạng thái</th><th class="text-end">Thao tác</th></tr></thead>
          <tbody>
            <tr v-for="item in filteredItems" :key="item.id">
              <td><div class="student-cell"><span class="avatar">{{ initials(item.studentName) }}</span><div><strong>{{ item.studentName }}</strong><small>{{ item.studentCode }}</small></div></div></td>
              <td><div class="course-cell"><strong>{{ item.courseTitle }}</strong><small>{{ item.courseCode }}</small></div></td>
              <td>{{ formatDate(item.enrolledAt) }}</td>
              <td><div class="progress-cell"><div class="progress"><div class="progress-bar" :style="{ width: `${item.progress}%` }"></div></div><small>{{ item.progress }}%</small></div></td>
              <td><strong>{{ item.finalScore ?? '—' }}</strong></td>
              <td><span :class="['badge', statusClass(item.status)]">{{ statusLabel(item.status) }}</span></td>
              <td class="text-end"><button v-if="item.status !== 'CANCELLED'" class="btn btn-light btn-sm text-danger" title="Hủy ghi danh" @click="cancelEnrollment(item)"><i class="bi bi-person-x"></i></button><span v-else class="text-secondary small">Có thể ghi danh lại</span></td>
            </tr>
            <tr v-if="!loading && !filteredItems.length"><td colspan="7" class="text-center text-secondary py-5">Chưa có dữ liệu ghi danh phù hợp.</td></tr>
          </tbody>
        </table>
      </div>
    ...
    <div v-if="formOpen" class="modal-mask" @click.self="formOpen = false">
      <form class="app-card enrollment-modal" @submit.prevent="saveEnrollment">
        ...
        <div class="student-picker">
          ...
          <div v-if="form.courseId && !availableStudents.length" class="empty-picker">Không còn học viên phù hợp. Học viên đã ghi danh sẽ không xuất hiện trong danh sách.</div>
        </div>
...
const filteredItems = computed(() => {
  ...
  return items.value.filter(item =>
    (!term || `${item.studentName} ${item.studentCode} ${item.courseTitle} ${item.courseCode}`.toLowerCase().includes(term)) &&
...
async function loadOptions() {
  try {
    ...
    students.value = (pick(studentData, 'items', 'Items') || []).map(row => ({ id: Number(pick(row, 'Id', 'id')), code: pick(row, 'StudentCode', 'studentCode') || '', name: pick(row, 'FullName', 'fullName') || '', email: pick(row, 'Email', 'email') || '' }))
    courses.value = (pick(courseData, 'items', 'Items') || []).map(row => ({ id: Number(pick(row, 'Id', 'id')), code: pick(row, 'Code', 'code') || '', title: pick(row, 'Title', 'title') || '' }))
  } catch (error) {
    show(error.message, 'danger')
  }
}
...
function initials(name = '') { return name.split(' ').filter(Boolean).slice(-2).map(value => value[0]).join('').toUpperCase() || 'HV' }
...
function statusLabel(value) { return { ENROLLED: 'Đã ghi danh', IN_PROGRESS: 'Đang học', COMPLETED: 'Hoàn thành', CANCELLED: 'Đã hủy' }[value] || value }
function statusClass(value) { return { ENROLLED: 'badge-soft-primary', IN_PROGRESS: 'badge-soft-warning', COMPLETED: 'badge-soft-success', CANCELLED: 'badge-soft-danger' }[value] || 'badge-soft-primary' }
