  ...
  <div :class="['cms-shell','cms-theme',{'sidebar-collapsed':sidebarCollapsed}]">
    ...
    <div class="cms-main">
      <header class="topbar bg-white d-flex align-items-center justify-content-between px-3 px-lg-4">
        <div class="topbar-heading d-flex align-items-center gap-3"><button class="btn btn-light d-lg-none menu-toggle" data-bs-toggle="offcanvas" data-bs-target="#mobileMenu" aria-label="Mở trình đơn"><i class="bi bi-list"></i></button><button type="button" class="sidebar-toggle d-none d-lg-grid" :aria-label="sidebarCollapsed ? 'Mở rộng thanh menu' : 'Thu gọn thanh menu'" :title="sidebarCollapsed ? 'Mở rộng menu' : 'Thu gọn menu'" @click="toggleSidebar"><svg class="sidebar-toggle-icon" viewBox="0 0 24 24" aria-hidden="true"><rect x="2.75" y="3.25" width="18.5" height="17.5" rx="3"></rect><path d="M8.25 3.5v17"></path><path v-if="sidebarCollapsed" d="m12 8 4 4-4 4"></path><path v-else d="m16 8-4 4 4 4"></path></svg></button><div><small>ELEARNING</small><div>{{ route.meta.title || sectionTitle }}</div></div></div>
        <div class="topbar-actions d-flex align-items-center gap-2 gap-lg-3"><form class="top-search d-none d-xl-flex" role="search" @submit.prevent="submitGlobalSearch"><i class="bi bi-search"></i><input v-model="globalSearch" type="search" aria-label="Tìm kiếm" placeholder="Tìm kiếm trong hệ thống..." autocomplete="off"><button type="submit" aria-label="Mở kết quả tìm kiếm"><i class="bi bi-arrow-right"></i></button></form><RouterLink class="icon-button d-xl-none" to="/cms/search" aria-label="Tìm kiếm"><i class="bi bi-search"></i></RouterLink><button class="icon-button" aria-label="Thông báo"><i class="bi bi-bell"></i><span></span></button><div class="d-none d-sm-flex align-items-center gap-2 user-summary"><img class="avatar user-avatar" :src="avatarUrl" :alt="`Ảnh đại diện ${auth.user?.fullName || ''}`"><div><div class="small fw-bold">{{ auth.user?.fullName }}</div><div class="tiny">{{ roleName }}</div></div></div><button class="btn btn-light btn-sm logout-button" title="Đăng xuất" @click="signOut"><i class="bi bi-box-arrow-right"></i></button></div>
      </header>
...
<script setup>
import { computed,onBeforeUnmount,ref,watch } from 'vue'; import { useRoute,useRouter } from 'vue-router'; import { useAuthStore } from '../stores/authStore'
...
function submitGlobalSearch(){const term=globalSearch.value.trim();router.push(term?{path:'/cms/search',query:{q:term}}:{path:'/cms/search'})}
...
watch(()=>route.query.q,value=>globalSearch.value=String(value||''))
</script>
