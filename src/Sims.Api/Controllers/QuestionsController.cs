using System.Security.Claims;using SimsObject.Common;using SimsObject.DTOs;using SimsObject.Interfaces;using Microsoft.AspNetCore.Authorization;using Microsoft.AspNetCore.Mvc;
...
public sealed class QuestionsController(IQuestionService questions):ControllerBase
{
 ...
 [HttpPut("{id:long}")] public async Task<ActionResult<ApiResponse<object>>> Update(long id,QuestionSaveRequest r,CancellationToken ct)=>await questions.UpdateAsync(id,r,UserId,IsAdmin,ct)?Ok(ApiResponse<object>.Ok(new{id})):NotFound(ApiResponse<object>.Fail("Không tìm thấy câu hỏi."));
 [HttpDelete("{id:long}")] public async Task<ActionResult<ApiResponse<object>>> Delete(long id,CancellationToken ct)=>await questions.DeleteAsync(id,UserId,IsAdmin,ct)?Ok(ApiResponse<object>.Ok(new{id})):NotFound(ApiResponse<object>.Fail("Không tìm thấy câu hỏi."));
}
