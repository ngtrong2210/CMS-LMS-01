...
public sealed class ContentController(IContentService content):ControllerBase
{
 ...
 [HttpGet("videos/{id:long}")] public async Task<ActionResult<ApiResponse<object>>> Video(long id,CancellationToken ct){var x=await content.GetVideoAsync(id,ct);return x is null?NotFound(ApiResponse<object>.Fail("Không tìm thấy video.")):Ok(ApiResponse<object>.Ok(x));}
 ...
 [HttpGet("video-library")] public async Task<ActionResult<ApiResponse<object>>> VideoLibrary([FromQuery]string? search,CancellationToken ct)=>Ok(ApiResponse<object>.Ok(await content.GetVideoLibraryAsync(search,ct)));
 ...
 [HttpGet("videos/{videoId:long}/interactions")] public async Task<ActionResult<ApiResponse<object>>> Interactions(long videoId,CancellationToken ct)=>Ok(ApiResponse<object>.Ok(await content.GetInteractionsAsync(videoId,ct)));
