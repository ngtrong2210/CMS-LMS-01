...
public sealed class ContentService(ISqlConnectionFactory connections, IVideoStorageService videoStorage) : IContentService
{
    ...
    public Task<long> CreateLessonAsync(long chapterId,LessonSaveRequest r,long actorId,bool isAdmin,CancellationToken ct=default)=>ScalarId("dbo.LMS_Lesson_Create",new{ChapterId=chapterId,r.Title,r.Description,r.LessonType,r.DurationSeconds,r.SortOrder,r.IsRequired,r.PassingScore,r.ContentHtml,r.DocumentUrl,r.AssignmentFolderName,r.DueAt,r.MaxSubmissionFileSizeMB,r.AllowLateSubmission,r.Status,ActorId=actorId,IsAdmin=isAdmin},ct);
    public Task<bool> UpdateLessonAsync(long id,LessonSaveRequest r,long actorId,bool isAdmin,CancellationToken ct=default)=>Affected("dbo.LMS_Lesson_Update",new{Id=id,r.Title,r.Description,r.LessonType,r.DurationSeconds,r.SortOrder,r.IsRequired,r.PassingScore,r.ContentHtml,r.DocumentUrl,r.AssignmentFolderName,r.DueAt,r.MaxSubmissionFileSizeMB,r.AllowLateSubmission,r.Status,ActorId=actorId,IsAdmin=isAdmin},ct);
