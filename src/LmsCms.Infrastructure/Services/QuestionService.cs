...
public sealed class QuestionService(ISqlConnectionFactory connections) : IQuestionService
{
    ...
    public Task<long> CreateAsync(QuestionSaveRequest request,long actorId,CancellationToken ct=default)=>Save(null,request,actorId,false,ct);
    public async Task<bool> UpdateAsync(long id,QuestionSaveRequest request,long actorId,bool isAdmin,CancellationToken ct=default){await Save(id,request,actorId,isAdmin,ct);return true;}
    public async Task<bool> DeleteAsync(long id,long actorId,bool isAdmin,CancellationToken ct=default){using var db=connections.CreateConnection();return await db.ExecuteScalarAsync<int>(new CommandDefinition("dbo.LMS_Question_Delete",new{Id=id,ActorId=actorId,IsAdmin=isAdmin},commandType:CommandType.StoredProcedure,cancellationToken:ct))>0;}
    private async Task<long> Save(long? id,QuestionSaveRequest r,long actorId,bool isAdmin,CancellationToken ct)
    {
        Validate(r);using var db=connections.CreateConnection();db.Open();using var tx=db.BeginTransaction();
        try{
            object parameters=id is null
                ? new{Id=id,r.QuestionType,r.QuestionText,r.Description,r.Explanation,r.Difficulty,r.DefaultScore,r.ShortAnswerMode,r.Status,ActorId=actorId}
                : new{Id=id,r.QuestionType,r.QuestionText,r.Description,r.Explanation,r.Difficulty,r.DefaultScore,r.ShortAnswerMode,r.Status,ActorId=actorId,IsAdmin=isAdmin};
            var questionId=await db.QuerySingleAsync<long>(new CommandDefinition(id is null?"dbo.LMS_Question_Create":"dbo.LMS_Question_Update",parameters,transaction:tx,commandType:CommandType.StoredProcedure,cancellationToken:ct));
