...
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE name='IX_VideoAssets_OwnerScope') CREATE INDEX IX_VideoAssets_OwnerScope ON dbo.VideoAssets(CreatedBy,ShareScope,Status,IsDeleted);
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE name='IX_VideoAssetShares_Teacher') CREATE INDEX IX_VideoAssetShares_Teacher ON dbo.VideoAssetShares(TeacherId,VideoAssetId);
